...
class ModelCore(Parameters):
    def __init__(self, n=30,
        ...
        self.spatiotemporal_precipitation = np.zeros((self.rows, self.columns))
    ...
    def _get_run_variables(self):
        """return dict containing all variables and values of the class instance 'self'
        ...
        --------
        dictionary: dict
            dict containg all instance attributes of self
        """
    ...
    def update_precipitation(self, t):
        """Modulates the initial precip dataset.

        if precipitation variation is true, the precipitation
        data is modulated with a 24 timestep period.

        Also, if veg_rainfall > 0, cleared_land_neighbours decreases rain.

        If the simulation time is withing one of the drought periods
        specified by self.drought_times, it total precipitation is reduced by
        the appropriate value in self.drought_severity globally.

        ToDo: Specialization:
        The original Model increases specialization every time
        ...
        # index of precipitation_variation:
        index = (np.ceil(t / self.climate_var) % 8).astype(int)

        # temporal modulation of rainfall
        temporal_modulation = self.precipitation_amplitude * self.precipitation_variation[index]

        # spatial modulation of rainfall due to cleared land neighbors
        spatial_modulation = self.veg_rainfall * self.cleared_land_neighbours

        # calculation of spatio-temporal precipitation data:
        if self.precipitation_modulation:
            self.spatiotemporal_precipitation = self.precip * (1 + temporal_modulation - spatial_modulation)
        else:
            self.spatiotemporal_precipitation = self.precip * (1 - spatial_modulation)
        ...
        # if so, decrease precipitation by factor percentage given by drought severity
        if drought:
            self.spatiotemporal_precipitation *= (1. - self.drought_severity / 100.)

    def get_waterflow(self):
        """calculate flow of water and remaining water on cells.
        ...
        rain_volume = np.nan_to_num(self.spatiotemporal_precipitation * 1e-5)
    ...
    def forest_evolve(self, npp):
        """update/evolce forest state on cells.

        Iterate 4 times over all cells and
        1) degenerate with probability of 0.003 * (1 + 2 * population_gradient)
        2) forest memory of cell += 1
        3) increase forest state, if forest memory is above threshold * npp_mean / npp[cell]
        4) increase to state 3 only if there are more than 3 neighboring cells in state 3 already.

        finally, calculate cleared land neighbors of all cells for update_precipitation.
        """
        ...
        for repeat in range(4):
            for i in self.list_of_land_patches:
                # ToDo check if this if clause is necessary. Shouldn't be according to specs of land_patches.
    ...
    def net_primary_prod(self):
        """
        net_primaty_prod is according to the Miamy Model
        ...
        pp. 237-263.  [doi:10.1007/978-3-642-80913-2_12]
        """
        npp = 3000 \
            * np.minimum(1 - np.exp(-6.64e-4
                                    * self.spatiotemporal_precipitation),
    ...
    def get_ecoserv(self, ag, wf):
        ...
        if not self.better_ess:
            ...
            self.es_sp = self.e_r * self.spatiotemporal_precipitation
    ...
    def benefit_cost(self, ag_in):
        """calculate production of cropped cell.
        Yes, the name is misleading, since this is only benefit. Costs come later.
        """
    ...
    def get_cropped_cells(self, bca):
        """Update each cities cropped cells.

        *   Abandon cells if population per cropped cell is lower than
            min_people_per_cropped_cell, if settlement_age > 5.
        *   Abandon cells with negative utiltiy,
        *   Crop new cells if population per cropped cell is lower than max_people_per_cropped cell.
            crop cells with highest utility first.
        *   If population per cropped cell is higher than max_people_per_cropped_cell,
            new cells are cropped.
        *   Crop cells in the cities 'influence' with highest utility.
        """
        abandoned = 0
        sown = 0
        ...
        for city in self.populated_cities:
            ...
            # consider only cells in the cities influence radius
            cells = list(zip(self.cells_in_influence[city][0],
                             self.cells_in_influence[city][1]))

            # for these cells, calculate utility of agriculture
            def util(x, y, c):

                u = bca[x, y]
                u -= self.estab_cost
                distance_from_city = np.sqrt((self.cell_width
                                              * (self.settlement_positions[0][c]
                                                 - self.coordinates[0][x, y])) ** 2
                                             + (self.cell_height
                                                * (self.settlement_positions[1][c]
            ...
            sorted_utility, sorted_available, sorted_cells = \
                list(zip(*sorted(list(zip(utility, available, cells)), reverse=True)))

            # of these sorted lists, filter only available cells
            available_util = list(compress(list(sorted_utility), list(sorted_available)))
            available_cells = list(compress(list(sorted_cells), list(sorted_available)))
            ...
            cropped_utils = [utility[cells.index(cell)] if cell in cells else -1 for cell in cropped_cells]

            # sort utilitites and cropped cells to lowest utilities first
            city_has_crops = True if len(cropped_cells) > 0 else False
            if city_has_crops:
                occupied_util, occupied_cells = zip(*sorted(list(zip(cropped_utils, cropped_cells))))

            # 1.) include new cells if population per cell exceeds threshold
    ...
    def get_pop_mig(self):
        ...
        Increase population of settlements according to fixed birth rate.
        Decrease population of settlements according to death rate, also depending on income pc.
        For each settlement, calculate number of people that are willing to leave.
        """

        # death rate correlates inversely with real income per capita
        death_rate_diff = self.max_death_rate - self.min_death_rate

        self.death_rate = [- death_rate_diff * self.real_income_pc[i] + self.max_death_rate
    ...
    def update_pop_gradient(self):
        """calculate population gradient

        The population gradient quantifies the disturbance of the forest by the population of settlements.
        population gradient on cell is limited below 15.
        """
        self.pop_gradient = np.zeros((self.rows, self.columns))
        for city in self.populated_cities:
    ...
    def evolve_soil_deg(self):
        ...
        Soil of cropped cells is degraded linearly with fixed erosion rate,
        Soil of cells with climax forest is regenerated with fixed regeneration rate
        """
    ...
    def get_rank(self):
        """classify settlements rank according to population.

        rank determines how many outgoing trade relationships a settlement can sustain.
        """
    ...
    def build_routes(self):
        """update topology of trade network.

        cities can establish and maintain outgoing trade links
        according to their rank that depends on their population.

        trade links can be longer, the higher the rank of a settlement.
        connect to neighbors in range with highest population.

        if settlements cannot sustain their trade links,
        they abandon links to smallest settlements first.
        """

        # update rank of settlements
        self.get_rank()

        # get degree of settlements
        ...
        for city in self.populated_cities:
            if self.degree[city] < self.rank[city]:
                ...
                # of those close, unconnected neighbors, connect to the one(s) with highest population.
                if sum(nearby) != 0:
                    try:
    ...
    def get_comps(self):
        """find connected components of trade network and calculate their size.
        """
    ...
    def get_centrality(self):
        """calculate centrality of settlements in trade network
        """
    ...
    def get_crop_income(self, bca):
        """calculate income from crops for each settlement.

        In origninal version of Mayasim, the income from crops
        was calculated as the mean over all cells,
        I thought this to be strange and calculate crop yield
        as the sum of yields from all cells of a settlement.
        """
    ...
    def get_eco_income(self, es):
        """calculate income from ecosystem services for all settlements.

        Like with crop income, in the original model income was calcualted
        as the mean of the income from all cells under the influence of a
        settlement.
        Also, like with crop income, I thought this to be strange and changed
        it to the sum over the ecosystem services from all cells under the
    ...
    def get_trade_income(self):
        """calculate income from trade of settlements depending of size of trade cluster and possition in it.
        """
        self.trade_income = [1. / 30. * (1 + self.comp_size[i] / self.centrality[i]) ** 0.9
    ...
    def get_real_income_pc(self):
        """calculate real income per capita for all settlements.
        Real income is the combination of income from agriculture, ecosystem services and trade.
        """
    ...
    def migration(self, es):
        """migrate population and create new settlements.

        for each settlement, if fraction of population that is willing to
        migrate exceeds 400, this part of population has prob. p=.5 to
        migrate and found a new settlement.

        Algorithm for foundation of new settlement is the following:
        * choose 75 cells on the map at random.
        * of these 75 chose cell with highest utility (ecosystem services - travel costs)
        * if for this cell there are no neighbors within r=7.5 radius, create settlement.
        * else, discart migrating population.
        """
        ...
        for city in self.populated_cities:
            rd = np.random.rand()
            if self.out_mig[city] > 400 and np.random.rand() <= 0.5:
                ...
                neighbours = \
                    (np.sqrt(self.area * ((new_loc[0] - self.settlement_positions[0]) ** 2 +
                                          (new_loc[1] - self.settlement_positions[1]) ** 2
                                          ))) <= 7.5
    ...
    def kill_cities(self):
        """kill cities depending on different criteria.

         Allways kill settlements that are smaller as minimum sustainable size.
         Depending on switch, kill settlements that have no cropped cells.

         Remove killed settlements from all lists of variables.
        """
    ...
    def update_trajectory_output(self, time, args, built, lost,
        ...
        self.trajectory.append([time,
                                ...
                                np.sum(self.s_es_sp),
                                np.sum(self.s_es_pg),
                                np.nanmean(self.spatiotemporal_precipitation),
                                np.nanmax(args[0]),
                                np.nanmean(args[1]),
                                np.nanmax(args[2]),
                                np.nanmax(args[3]),
