...
###EQUATION###################################################################            
        self.spaciotemporal_precipitation = self.precip*(1 
            + self.precipitationModulation[(np.ceil(t/self.climate_var)%8).astype(int)]) - self.veg_rainfall*self.cleared_land_neighbours
###EQUATION###################################################################            
    ...
    def forest_evolve(self, npp):
        ...
        for repeat in xrange(4):
            for i in self.list_of_land_patches: 
                ...
                if (self.forest_state[i] == 1 and self.forest_memory[i] > self.state_change_s2*threshold):
                    self.forest_state[i] = 2
                    self.forest_memory[i] = self.state_change_s2
                ### recover if tree = 2 and memory > threshold 2 
                ### and certain number of neighbours are climax forest as well
                if (self.forest_state[i] == 2 and self.forest_memory[i] > self.state_change_s3*threshold):
    ...
    def get_ag(self,npp,wf):
        ...
        ### of each patch.
###EQUATION###################################################################            
        return self.a_npp*npp + self.a_sp*self.soilprod - self.a_s*self.slope - self.a_wf*wf - self.soil_deg
###EQUATION###################################################################            
...
###EQUATION###################################################################            
        return self.e_ag*ag + self.e_wf*wf + self.e_f*(self.forest_state-1.) #+ self.e_r*rain(t) - self.e_deg * pop_gradient
###EQUATION###################################################################            
    ...
    def get_cropped_cells(self,bca):
        ...
        for city in self.populated_cities:
            distances = np.sqrt(self.area*(
                ...
                (self.settlement_positions[1][city] - self.coordinates[1])**2))
###EQUATION###################################################################            
            utility = bca - self.estab_cost - (self.ag_travel_cost * distances)/np.sqrt(self.population[city])
###EQUATION###################################################################            

            # 1.) abandon cells if population too low after cities age > 5 years
            if (ag_pop_density[city] < self.min_people_per_cropped_cell and self.age[city] > 5):
            ...
            for number_new_cells in xrange(np.floor(ag_pop_density[city]/self.max_people_per_cropped_cell).astype('int')):   
    ...
    def get_pop_mig(self):
        ...
        self.death_rate = [-death_rate_diffe * self.real_income_pc[i] + self.max_death_rate for i in range(len(self.real_income_pc))]
        self.death_rate = [self.min_death_rate if value<self.min_death_rate else self.max_death_rate \
                if value>self.max_death_rate else self.death_rate[i] \
                for i, value in enumerate(self.death_rate)]
        
        # population control
        if self.population_control == True:
            self.birth_rate = [-(self.max_birth_rate - self.min_birth_rate)/10000. * value + self.shift if value > 5000 else self.birth_rate_parameter for value in self.population]
...
###EQUATION###################################################################            
        ...
        self.trade_income = [self.r_trade if value>1 else 0 if (value<0 or self.degree[index]==0) else self.r_trade*value for index, value in enumerate(self.trade_income)]
###EQUATION###################################################################            
        return
